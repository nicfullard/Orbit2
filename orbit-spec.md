...
- **Same project, any department** — the same visibility rule as for parents: both ends of a link must be on the same project (or both standalone in the same department), so the only titles a link exposes are ones the viewer can already see on that project. Cross-project links are not supported in v1.
