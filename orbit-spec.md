...
**Drawn by the Gantt view (§6.16):** the chart itself and summary spans for parents computed from their children. The Gantt also computes the critical path and lets a task's planned dates be dragged. **Still deferred:** automatic scheduling — shifting a successor's dates when a predecessor's move, or a lag, breaks a link. Everything above is what the view reads; nothing here changes shape for it.
...
- **Critical path:** the chain of tasks that sets the project's end, computed the classic way over the drawn tasks: durations from the planned dates (one day for a single-date task), each planned start as an earliest-start constraint, all four link types and lags honoured in a forward and a backward pass, cancelled tasks left out. A task with zero total float is critical, and a link is *driving* when it is what fixes its successor's earliest date. Critical bars get an outline and a *CP* mark beside the status badge, driving links between critical tasks a heavier arrow, and the header says how many tasks are on the path and when it ends. Recomputed on every load, never stored.
...
> **Assumptions flagged:** (1) dragging changes only the dragged task — no automatic shifting of successors; a conflict is shown, not fixed. (2) The critical path treats each planned start as an earliest-start constraint, so a task planned earlier than its links allow is scheduled where the links put it, not where it is drawn; done tasks stay in the calculation (they are part of the chain), cancelled ones don't. (3) Summary spans are derived from children, not stored; a parent's own dates are drawn as its own bar when set. (4) One project per chart; a cross-project or company-wide timeline isn't offered.
...
| `get_project_status` | Lightweight status summary only — task counts by status, overdue, progress, time logged and estimated (§6.10) and a per-department breakdown (each department's counts by status, overdue, open/total and done-%, matching the project page's table) — good for a quick "how's project X doing" check without pulling every task. |
