...
| Move a task between `Todo` / `InProgress` / `Waiting` / `Blocked` | ✅ (own department) | ✅ (own department) | ✅ (any department) |
...
| `update_project` | Edit a project — name, description, status, owner, target date, required project buffer (`none` clears). Same field-level rules as the Razor Pages UI (§6.1); no separate close-restriction applies here since project status isn't gated like task closing is. A `DepartmentAdmin` key is rejected if the project isn't in its own department. |
