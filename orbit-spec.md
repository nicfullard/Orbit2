...
| Tool | Purpose |
|---|---|
| `create_task` | Create a task. Args: title, description, projectId (optional), departmentId (optional — with a projectId it defaults to the project's department; otherwise to the key's own department for a `DepartmentAdmin`/`Member` key, and is required for a `SystemAdmin` key with no department, since the task needs one), priority, startDate (optional — the planned start, not after dueDate), dueDate, assigneeId (optional), parentTaskId (optional — makes the new task a subtask of a task on the same project, §6.15), idempotencyKey (optional). A `SystemAdmin` key may pass a departmentId that differs from the project's to file a cross-department project task (§6.2.1); any other key is rejected for that. `Source` is forced to `Api` server-side. |
