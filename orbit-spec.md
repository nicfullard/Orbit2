...
- **`Blocked` stays manual.** Waiting on a predecessor is a computed condition, not a status: `Blocked` keeps meaning "held up by something outside the plan". The computed state is shown as a **Waiting** badge and describes the task's *next* move — a `Todo` task with an unmet FS/SS link, or an open task with an unmet FF/SF link. A task that has already started isn't retroactively "waiting" if its FS predecessor is reopened.
