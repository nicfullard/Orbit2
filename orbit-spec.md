...
- **Axis:** from the earliest planned date (or today) to the latest, with a few days' padding, in day columns: month labels always, day numbers when there is room, otherwise week starts; weekends shaded; a **Today** line. The day width steps down as the range grows (about a month and a half at full width, a year still scrollable on one screen).
...
- Used only by the analysis — to count spans, float, buffer and lag in working days — and for readiness warnings. Nothing else moves because of it: a task planned across a non-working day stays put and is pointed out.
...
| `list_tasks` | List tasks. Args: `projectId`, `departmentId`, `status`, `assigneeId`, `source`, `dueBefore`/`dueAfter`, `plannedFor` (a date or `today` — the day plan, §6.12), `parentTaskId` (one task's direct subtasks, §6.15). Paginated. A `DepartmentAdmin`/`Member` key is scoped to its own department regardless of the `departmentId` arg; a `SystemAdmin` key can query any department or omit the filter for all. |
